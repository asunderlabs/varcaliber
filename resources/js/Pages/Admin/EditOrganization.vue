<template>
    ...
    <BreezeAuthenticatedLayout :isAdmin="true">
        <div class="bg-white px-4 md:px-6 py-3">
            <PageSection header="Edit Organization">
                <form @submit.prevent="onSubmit" class="space-y-2">
                    ...
                    <div>
                        ...
                        <input type="text" :value="organization.client_id" class="w-full bg-gray-100 text-gray-800" readonly />
                    </div>
                    <div>
                    ...
                    <button type="submit" class="rounded bg-indigo-500 text-white p-2">Save Organization</button>
                </form>
            </PageSection>
        </div>
...
export default {
    ...
    created() {
        ...
        this.form.stripe_customer_id = this.organization.stripe_customer_id
        this.form.billing_contact = this.organization.billing_contact
        this.form.email = this.organization.email
        this.form.address_line_1 = this.organization.address_line_1
        this.form.address_line_2 = this.organization.address_line_2
        this.form.city = this.organization.city
        this.form.state = this.organization.state
        this.form.zip_code = this.organization.zip_code
        this.form.hourly_rate = this.organization.hourly_rate
    },

    methods: {
        onSubmit(e) {
            this.$inertia.put(this.route('admin.organizations.update', this.organization.id), this.form)
        },
    },
}
</script>
